CLASS zak_cl_generate_data DEFINITION
  PUBLIC
  FINAL
  CREATE PUBLIC .
...
CLASS ZAK_CL_GENERATE_DATA IMPLEMENTATION.


  METHOD if_oo_adt_classrun~main.

*    DATA: lt_prod_grs  TYPE TABLE OF zak_d_prod_group,
*          lt_phases    TYPE TABLE OF zak_d_phase,
*          lt_markets   TYPE TABLE OF zak_d_market,
*          lt_uom       TYPE TABLE OF zak_d_uom.
*
**** PRODUCT GROUPS
...
*    DELETE FROM zak_d_prod_group.
**   insert the new table entries
*    INSERT zak_d_prod_group FROM TABLE @lt_prod_grs.
*
**   check the result
*    SELECT * FROM zak_d_prod_group INTO TABLE @lt_prod_grs.
*    out->write( sy-dbcnt ).
...
*
**   Delete the possible entries in the database table - in case it was already filled
*    DELETE FROM zak_d_phase.
**   insert the new table entries
*    INSERT zak_d_phase FROM TABLE @lt_phases.
*
**   check the result
*    SELECT * FROM zak_d_phase INTO TABLE @lt_phases.
*    out->write( sy-dbcnt ).
...
*
**   Delete the possible entries in the database table - in case it was already filled
*    DELETE FROM zak_d_market.
**   insert the new table entries
*    INSERT zak_d_market FROM TABLE @lt_markets.
*
**   check the result
*    SELECT * FROM zak_d_market INTO TABLE @lt_markets.
*    out->write( sy-dbcnt ).
...
*
**   Delete the possible entries in the database table - in case it was already filled
*    DELETE FROM zak_d_uom.
**   insert the new table entries
*    INSERT zak_d_uom FROM TABLE @lt_uom.
*
**   check the result
*    SELECT * FROM zak_d_uom INTO TABLE @lt_uom.
*    out->write( sy-dbcnt ).
*    out->write( 'uom data inserted successfully!').

*    delete from zak_d_prod_mrkt.
*    delete from zak_d_product.
*    update zak_d_prod_mrkt set status = 'X' where mrkt_uuid = '122EF101E67B1EDCB9B5707B00B1BB8F'.
*    update zak_d_market set imageurl = 'https://cdn.countryflags.com/thumbs/spain/flag-400.png' where mrktid = 10.
*    update zak_d_product set phaseid = 1 pgid = '3' where prod_uuid = '122EF101E67B1EDCB9B4AEABB4617A8B'.
*    update zak_d_product set phaseid = 2 where prod_uuid = '12E36AA965EF1EDCB9CAF653DF27A5DA'.
